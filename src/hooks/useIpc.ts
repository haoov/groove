...
import { OP, OP_GIT_PREFIX, OP_MR_PREFIX } from '../lib/ops';
...
import { disposeHost } from '../components/terminalHost';
import type {
  AgentActivity,
  Annotation,
  Task,
...
// Coalesce a burst of filesystem `file_changed` events into one diff refresh.
const FILE_CHANGED_DEBOUNCE_MS = 350;
let fileChangedTimer: number | null = null;
// Paths accumulated across the current debounce window, so the flush can target
// only the sessions whose worktrees actually changed.
const pendingChangedPaths = new Set<string>();

/** Ask the backend to (idempotently) start filesystem watchers for a task's
 *  worktrees, so subsequent edits auto-refresh the diff. */
function startWatchers(taskId: string) {
  invoke('watch_task_worktrees', { taskId }).catch(() => { /* best-effort */ });
}
    ...
    const setup = async () => {
      ...
      track(
        await listen<WorkspaceReadyEvent>(EVENT.WORKSPACE_READY, ({ payload }) => {
          ...
          // Watch the worktrees so edits auto-refresh the diff (item: auto-sync).
          if (payload.task?.short_id) startWatchers(payload.task.short_id);
        })
      );
      ...
      // file_changed — a worktree's working tree changed on disk (agent edit,
      // terminal command, external tool). Coalesce a burst into one refresh, then
      // refresh only the sessions whose worktrees own the changed paths.
      track(
        await listen<FileChangedEvent>(EVENT.FILE_CHANGED, ({ payload }) => {
          if (fileChangedTimer !== null) clearTimeout(fileChangedTimer);
          for (const p of payload.paths ?? []) pendingChangedPaths.add(p);
          fileChangedTimer = window.setTimeout(() => {
            fileChangedTimer = null;
            const paths = [...pendingChangedPaths];
            pendingChangedPaths.clear();
            const s = useStore.getState();
            const owners = s.sessionOrder.filter((id) =>
              s.sessions[id]?.worktrees.some((w) =>
                paths.some((p) => p === w.path || p.startsWith(w.path + '/'))
              )
            );
            // Nothing matched (empty payload, or a path outside any worktree) —
            // fall back to refreshing every session.
            const targets = owners.length ? owners : s.sessionOrder;
            for (const id of targets) {
              s.invalidateDiff(id);
              s.refreshStatusFor(id);
            }
            // Home mirrors working-tree state, so it goes stale on the same
            // edits — but only refresh it while it's actually on screen.
            if (s.view !== 'workspace') s.refreshHome();
