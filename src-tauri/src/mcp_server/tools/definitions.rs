...
pub(crate) fn mcp_tool_definitions() -> Vec<serde_json::Value> {
    vec![
        ...
        mcp_tool("list_repos", "Every repo cloned locally, each flagged `attached` when it is already on your task, and each with the `local_path` of its clone. Use it for the exact name add_task_repo wants, or for the path to read a repo this session has not checked out.", serde_json::json!({"type":"object","properties":{}})),
        mcp_tool("get_task_diff", "Diff for all worktrees of a task (vs origin/main; for explorer sessions: the uncommitted working-tree changes).", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."}}})),
        mcp_tool("get_commit_log", "Commit history for all worktrees of a task.", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."},"limit":{"type":"integer"}}})),
        ...
        mcp_tool("get_mr_threads", "The discussion on an MR, live from the forge: review threads with their file and line, and general comments such as a CI bot's report. Each note says who wrote it and whether its thread is resolved. A reviewer's thread is a note to act on, the same as an annotation.", serde_json::json!({"type":"object","required":["mr_id"],"properties":{"mr_id":{"type":"string","description":"The MR's id from get_mr_state."}}})),
        ...
        mcp_tool("get_annotations", "All annotations for a task.", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."}}})),
        ...
        mcp_tool("get_task_schema", "The task's properties, live from the source: each one's kind, whether it is editable, the exact options a select or status accepts, and the value it holds now (null means unset). Read it before update_task_property — the property name and the value both have to match what the source spells, and nothing else tells you either. An unset property is one to set when the work gives it a value, not one to skip.", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."}}})),
        mcp_tool("list_relation_options", "The pages a relation property can point to, as id and title. Read it before setting a relation with update_task_property, which takes page ids. Empty when the property is not a relation or its target has no pages.", serde_json::json!({"type":"object","required":["property"],"properties":{"property":{"type":"string","description":"Exact property name, as the task's source spells it."},"task_id":{"type":"string","description":"Defaults to your own task."}}})),
        ...
        mcp_tool("get_task_time", "Time Groove measured for a task: tracked, already logged, and `unlogged_hours` — the part not yet at the source. That is what log_task_hours should add; logging it twice double-counts.", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."}}})),
        mcp_tool("finish_task", "Close the task: mark it done at its source, then DELETE every worktree of this session and its local rows. Requires confirmation. Destructive and not undoable — check first that nothing is uncommitted, unpushed, or waiting on an MR. It also ENDS this session and stops you, so say everything you have to say before calling it: you will not see the result.", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."}}})),
