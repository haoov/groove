...
pub(crate) fn mcp_tool_definitions() -> Vec<serde_json::Value> {
    vec![
        ...
        mcp_tool("get_task_schema", "The task source's live schema: every property with its kind, whether it is editable, and the exact options a select or status accepts. Read it before update_task_property — the property name and the value both have to match what the source spells, and nothing else tells you either.", serde_json::json!({"type":"object","properties":{"task_id":{"type":"string","description":"Defaults to your own task."}}})),
