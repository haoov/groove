...
pub(crate) fn mcp_tool_definitions() -> Vec<serde_json::Value> {
    vec![
        ...
        mcp_tool("update_mr", "Update MR title/description. Requires confirmation.", serde_json::json!({"type":"object","required":["mr_id"],"properties":{"mr_id":{"type":"string"},"title":{"type":"string","description":SUBJECT},"description":{"type":"string","description":mr_description()}}})),
        mcp_tool("add_task_repo", "Attach a repo to your task and create its worktree. Requires confirmation. The repo must already be cloned under MAIN — call list_repos first; this tool cannot clone.", serde_json::json!({"type":"object","required":["repo"],"properties":{"repo":{"type":"string","description":"Slug (group/path/project) or project name from list_repos."},"task_id":{"type":"string","description":"Defaults to your own task."},"branch":{"type":"string","description":"Branch for the new worktree. Defaults to the task branch."}}})),
        ...
        mcp_tool("get_task_template", "Fetch the task template as markdown (template_markdown), empty when the source has none. Mirror its headings when drafting body_markdown for create_task_from_explorer — but keep each section to a line or two.", serde_json::json!({"type":"object","properties":{"provider":{"type":"string","description":"notion or github. Only needed when more than one source is set up and the session has no task."}}})),
