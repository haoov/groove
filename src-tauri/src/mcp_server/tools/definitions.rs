...
pub(crate) fn mcp_tool_definitions() -> Vec<serde_json::Value> {
    vec![
        ...
        mcp_tool("update_notion_status", "Update Notion task status. Requires confirmation.", serde_json::json!({"type":"object","required":["notion_page_id","status"],"properties":{"notion_page_id":{"type":"string"},"status":{"type":"string"},"status_prop_name":{"type":"string"}}})),
