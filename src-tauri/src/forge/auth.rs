...
/// hosts as headings with indented "✓ …" lines; the token line contains
/// "Token:" followed by the token.
fn glab_token_for(output: &str, host: &str) -> Option<String> {
    let mut in_host = false;
    for line in output.lines() {
        let trimmed = line.trim();
        // A heading is the bare host name (no checkmark, not indented content).
        ...
        if let Some(at) = trimmed.find("Token:") {
            let token = trimmed[at + "Token:".len()..].trim();
...
mod tests {
    ...
    /// Captured from glab 1.55 — hosts as headings, details indented, token shown
    /// with `-t`.
    const STATUS: &str = "\
gitlab.wiremind.io
  ✓ Logged in to gitlab.wiremind.io as rsabbah (/home/r/.config/glab-cli/config.yml)
  ✓ Git operations for gitlab.wiremind.io configured to use ssh protocol.
  ✓ API calls for gitlab.wiremind.io are made over https protocol
  ✓ Token: glpat-AAAAbbbbCCCCdddd
gitlab.com
  x gitlab.com: api call failed
  ✓ Token: glpat-OTHERtokenHERE
";
    ...
    fn finds_the_token_of_the_asked_host_only() {
        assert_eq!(
            ...
            Some("glpat-AAAAbbbbCCCCdddd")
        );
        assert_eq!(
            glab_token_for(STATUS, "gitlab.com").as_deref(),
            Some("glpat-OTHERtokenHERE")
        );
