...
//!
//! Branches follow `<type>/<id>-<slug>`; the type is inferred from the task
...
mod tests {
    ...
    #[test]
    fn task_branches_follow_type_id_slug() {
        let s = session(SessionKind::Task, "TASKS2-42", "Fix the diff parser crash");
        assert_eq!(default_branch(&s), "fix/tasks2-42-fix-the-diff-parser-crash");

        let s = session(SessionKind::Task, "TASKS2-43", "Add MR templates");
        assert_eq!(default_branch(&s), "feat/tasks2-43-add-mr-templates");
    }
    ...
    fn slugs_are_bounded_and_never_cut_words() {
        ...
        assert!(branch.starts_with("feat/tasks2-1-implement-the"), "{branch}");
        assert!(!branch.ends_with('-'), "{branch}");
